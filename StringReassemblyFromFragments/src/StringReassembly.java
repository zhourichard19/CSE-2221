...
public final class StringReassembly {
    ...
    public static String combination(String str1, String str2, int overlap) {
        ...
        //makes a string from beginning of the first string until the overlap
        String cutString = str1.substring(0, str1.length() - overlap);
        //adds the first string until the overlap to the second string
        String combination = cutString + str2;

        //gets the new cut string and return it
        return combination;
    }
    ...
    public static void addToSetAvoidingSubstrings(Set<String> strSet,
        ...
        //checks to make sure that the second loop will be run through
        boolean secondLoop = true;

        //loops through every string in the strSet and checks to see if
        //s is a substring of str
        for (String s : strSet) {
            if (s.indexOf(str) >= 0) {
                secondLoop = false;
            }
        ...
        //checks to see if the second loops will need to go through
        if (secondLoop) {
            //creates a new instance variable that is empty
            Set<String> temp = strSet.newInstance();
            //goes through every string in strSet
            for (String s : strSet) {
                //if the string is not a subset, then add to the instance
                //variable temp
                if (str.indexOf(s) == -1) {
                    temp.add(s);
                }
            }
            //transfers temp to strSet which is returned
            strSet.transferFrom(temp);
            //then adds the final bit to strSet which is str.
            strSet.add(str);
        }
    }
    ...
    public static Set<String> linesFromInput(SimpleReader input) {
        ...
        //creates a new string set variable to hold the return value
        Set<String> strSet = new Set1L<>();
        //creates a new string which what is next in the set
        String str1 = input.nextLine();

        //adds the input
        strSet.add(str1);

        //checks to make sure there is still things in the stream until
        //there is none
        while (!input.atEOS()) {
            //goes through and saves inputs to the variable string
            String str = input.nextLine();
            //adds the string to the full thing by calling the previous method
    ...
    public static void printWithLineSeparators(String text, SimpleWriter out) {
        ...
        //goes through the text and reads every character
        for (int i = 0; i < text.length(); i++) {
            ...
            if (text.charAt(i) == '~') {
                ...
                //if it is not a ~ then just prints out the letter
